...
const state = {
  ...
  formulaStatusPollTimer: null,
  formulaStatusRefreshing: false,
  formulaDownloadInProgress: false,
  formulaModelsReady: false,
...
function updateConvertOptions() {
  ...
  if (formulaOptions) {
    ...
    if (fmt === 'docx') refreshFormulaStatus()
  }
}
...
$('btn-convert').addEventListener('click', async () => {
  ...
  try {
    ...
    if (fmt === 'docx') {
      payload.formula_ocr = !!$('cv-formula-ocr')?.checked
      if (payload.formula_ocr && !state.formulaModelsReady) {
        setFormulaStatus('公式模型未就绪，将自动回退为普通公式版面')
      }
    }
...
function stopFormulaStatusPolling() {
  if (!state.formulaStatusPollTimer) return
  clearInterval(state.formulaStatusPollTimer)
  state.formulaStatusPollTimer = null
}
...
function startFormulaStatusPolling() {
  stopFormulaStatusPolling()
  state.formulaStatusPollTimer = setInterval(async () => {
    const ready = await refreshFormulaStatus()
    if (ready || !state.formulaDownloadInProgress) {
      stopFormulaStatusPolling()
    }
  }, 1200)
}
...
function setFormulaActionButtonsEnabled(enabled) {
  const btn = $('btn-formula-download')
  if (btn) btn.disabled = !enabled || state.formulaDownloadInProgress
}

async function refreshFormulaStatus() {
  if (!window.pdfAPI?.formulaStatus || !$('formula-status')) return false
  if (state.formulaStatusRefreshing) return !!state.formulaModelsReady
  state.formulaStatusRefreshing = true
  try {
    const status = await window.pdfAPI.formulaStatus()
    const runtimeReady = !!status.runtime_ready
    const modelsReady = !!status.models_ready
    const downloading = !!status.downloading
    const error = status.error || ''
    const message = status.message || ''
    state.formulaModelsReady = modelsReady

    if (!runtimeReady) {
      state.formulaDownloadInProgress = false
      setFormulaActionButtonsEnabled(false)
      setFormulaStatus(normalizeOcrRuntimeMessage(error) || '公式识别运行库未就绪')
      return false
    }
    setFormulaActionButtonsEnabled(true)
    if (downloading) {
      setFormulaStatus(message || '公式模型下载中...')
      return false
    }
    if (modelsReady) {
      state.formulaDownloadInProgress = false
      setFormulaStatus('公式识别模型已就绪')
      return true
    }
    state.formulaDownloadInProgress = false
    setFormulaStatus(error ? `公式识别模型未下载：${simplifyOcrError(error)}` : (message || '公式识别模型未下载'))
    return false
  } catch (e) {
    state.formulaDownloadInProgress = false
    setFormulaStatus('公式状态获取失败：' + (e.message || e))
    return false
  } finally {
    state.formulaStatusRefreshing = false
  }
}
...
$('btn-formula-download')?.addEventListener('click', async () => {
  if (!window.pdfAPI?.formulaDownload) return
  if ($('btn-formula-download')?.disabled) {
    await refreshFormulaStatus()
    return
  }
  setFormulaStatus('正在请求下载公式识别模型...')
  state.formulaDownloadInProgress = true
  setFormulaActionButtonsEnabled(true)
  try {
    await window.pdfAPI.formulaDownload({ model: 'PP-FormulaNet_plus-M' })
    startFormulaStatusPolling()
    await refreshFormulaStatus()
  } catch (e) {
    state.formulaDownloadInProgress = false
    stopFormulaStatusPolling()
    setFormulaStatus('公式模型下载启动失败：' + (e.message || e))
    await refreshFormulaStatus()
  }
})
